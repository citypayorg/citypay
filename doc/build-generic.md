GENERIC BUILD NOTES
====================
Some notes on how to build Ctp Core based on the [depends](../depends/README.md) build system.

Note on old build instructions
------------------------------
In the past, the build documentation contained instructions on how to build Ctp with system-wide installed dependencies
...
------------------------------------
Building the dependencies and Ctp Core requires some essential build tools to be installed before. Please see
[build-unix](build-unix.md), [build-osx](build-osx.md) and [build-windows](build-windows.md) for details.

Building dependencies
---------------------
Ctp inherited the `depends` folder from Bitcoin, which contains all dependencies required to build Ctp. These
dependencies must be built before Ctp can actually be built. To do so, perform the following:
...
This will download and build all dependencies required to build Ctp Core. Caching of build results will ensure that only
...
not done, build failures might occur when building Ctp.
...
level. `./configure` of Ctp Core will autodetect the presence of ccache and enable use of it. To disable ccache, use
